...
def after_web(state: AgentState) -> Literal[
    "generate_from_web",
    "rewrite_query",
    "insufficient"
]:
...
Answer ONLY from the private company HR KB below.
Be concise, practical, respectful, and policy-grounded.
...
def generate_from_web(state: AgentState):
    answer = llm().invoke(f"""
You are an enterprise HR policy and employee support copilot.
The private company HR KB was insufficient.

Answer ONLY from the web evidence below.
Clearly say this is external public information and may require HR validation before being treated as company policy or employment guidance.
